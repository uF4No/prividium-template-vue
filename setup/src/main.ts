...
async function main() {
    ...
    const ssoContracts = await deploySsoContracts({
        ...
        configured: {
            ...
            entryPoint:
                ...
                (extractConfigOptional(setupEnvPath, 'SSO_ENTRYPOINT_CONTRACT') as `0x${string}` | undefined),
    ...
    console.log(`  WebAuthn validator: ${ssoContracts.webauthnValidator} (${ssoContracts.deployed.webauthnValidator ? 'deployed' : 'existing'})`);
    console.log(`  EOA validator: ${ssoContracts.eoaValidator} (${ssoContracts.deployed.eoaValidator ? 'deployed' : 'existing'})`);
    console.log(`  Session validator: ${ssoContracts.sessionValidator} (${ssoContracts.deployed.sessionValidator ? 'deployed' : 'existing'})`);
    console.log(`  Guardian executor: ${ssoContracts.guardianExecutor} (${ssoContracts.deployed.guardianExecutor ? 'deployed' : 'existing'})`);
    console.log(`  EntryPoint: ${ssoContracts.entryPoint} (${ssoContracts.deployed.entryPoint ? 'deployed' : 'existing'})`);
    console.log(`  Account implementation: ${ssoContracts.accountImplementation} (${ssoContracts.deployed.accountImplementation ? 'deployed' : 'existing'})`);
    console.log(`  Beacon: ${ssoContracts.beacon} (${ssoContracts.deployed.beacon ? 'deployed' : 'existing'})`);
    console.log(`  Factory: ${ssoContracts.factory} (${ssoContracts.deployed.factory ? 'deployed' : 'existing'})`);
