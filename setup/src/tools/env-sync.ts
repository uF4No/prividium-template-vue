...
export function syncEnvFromContractsConfig(args: SyncEnvArgs) {
    ...
    if (config.sso?.webauthnValidator) {
        setDotEnvConfig(args.backendPath, 'SSO_WEBAUTHN_VALIDATOR_CONTRACT', config.sso.webauthnValidator);
        setDotEnvConfig(args.webAppPath, 'VITE_SSO_WEBAUTHN_VALIDATOR', config.sso.webauthnValidator);
    }
    if (config.sso?.sessionValidator) {
        setDotEnvConfig(args.backendPath, 'SSO_SESSION_VALIDATOR_CONTRACT', config.sso.sessionValidator);
    }
    if (config.sso?.guardianExecutor) {
        setDotEnvConfig(args.backendPath, 'SSO_GUARDIAN_EXECUTOR_CONTRACT', config.sso.guardianExecutor);
    }
    ...
    if (config.sso?.accountImplementation) {
        setDotEnvConfig(args.backendPath, 'SSO_ACCOUNT_IMPLEMENTATION_CONTRACT', config.sso.accountImplementation);
    }
