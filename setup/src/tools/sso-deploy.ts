...
export async function deploySsoContracts(config: SsoDeployConfig): Promise<SsoDeploymentResult> {
    ...
    async function ensureBeacon(): Promise<{
        ...
        if (configured) {
            ...
            return { address: configured, implementation, deployed: false, implementationDeployed: false };
        }

        const { address: implementation, deployed: implementationDeployed } = await ensureAccountImplementation();
    ...
    async function ensureFactory(beaconAddress: Address): Promise<{ address: Address; deployed: boolean }> {
    ...
    async function ensureEntryPoint(): Promise<{ address: Address; deployed: boolean }> {
        ...
        if (!configured) {
            throw new Error('Missing entryPoint address. Set SSO_ENTRYPOINT_CONTRACT in contracts config.');
        }
    ...
    const guardianExecutor = await ensureGuardianExecutor(webauthnValidator.address, eoaValidator.address);
