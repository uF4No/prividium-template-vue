...
async function deployAccountWithoutSDK(originDomain: string, credentialId: string, credentialPublicKey: number[]) {
  ...
  const findSdkPackageDir = () => {
    ...
    for (const base of tryDirs) {
      ...
      if (existsSync(pnpmDir)) {
        const matches = readdirSync(pnpmDir).filter((name) => name.startsWith("zksync-sso-web-sdk@"));
        if (matches.length > 0) {
          const pkgJson = join(pnpmDir, matches[0], "node_modules", "zksync-sso-web-sdk", "package.json");
  ...
  if (!ssoPkgDir) {
    throw new Error("zksync-sso-web-sdk package not found. Ensure workspace dependencies are installed.");
  }

  const ssoWasmPath = join(ssoPkgDir, "pkg-node", "zksync_sso_erc4337_web_ffi.js");
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const ssoWasm = require(ssoWasmPath) as {
    PasskeyPayload: new (credentialId: Uint8Array, passkeyX: Uint8Array, passkeyY: Uint8Array, originDomain: string) => unknown;
