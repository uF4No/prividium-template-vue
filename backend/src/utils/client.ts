...
async function getAuthToken(): Promise<string> {
  ...
  const run = async () => {
    try {
      // 1. Request Challenge
      const authBaseUrl =
        env.PRIVIDIUM_AUTH_BASE_URL || env.CORS_ORIGIN || "http://localhost:3001";
      ...
      if (!loginRes.ok) {
        ...
        throw new Error(
          `Failed to login: ${loginRes.status} ${loginRes.statusText} ${errorText}`,
        );
      }
...
const l2 = createPublicClient({
  ...
  transport: http(L2_RPC_URL, {
    fetchFn: authenticatedFetch as any,
  }),
});
