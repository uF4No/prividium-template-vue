...
export type DeployAccountRouterDeps = {
  deploySmartAccount: typeof deploySmartAccount;
};
...
export function createDeployAccountRouter(deps: Partial<DeployAccountRouterDeps> = {}): Router {
  ...
  router.post('/', async (req: Request, res: Response) => {
    ...
    if (!parsed.success) {
      ...
      const deployResult = await deploySmartAccountFn(
        parsed.data.userId,
        parsed.data.originDomain,
        parsed.data.credentialId,
        parsed.data.credentialPublicKey
      );
...
export const deployAccountRouter = createDeployAccountRouter();
