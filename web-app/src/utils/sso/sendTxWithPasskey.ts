...
export async function sendTxWithPasskey(
  ...
  const structHash = keccak256(
    encodeAbiParameters(parseAbiParameters("bytes32,address,uint256,bytes32,bytes32,bytes32,uint256,bytes32,bytes32"), [
  ...
  const userOpHashFromBundler = (await (readClient as any).request({
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const receiptResult = await (readClient as any).request({
      method: "eth_getUserOperationReceipt",
      params: [userOpHashFromBundler],
    });

    if (receiptResult) {
      receipt = receiptResult as any;
      break;
    }
  }
